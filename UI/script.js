...
function getArticles(skip, top, filterConfig){
	...
	var articlesNew =  articles.filter(function(elem){
		if(filterConfig != 0){
			if(filterConfig.author == elem.author) return true;
			if(filterConfig.createdAt == elem.createdAt) return true;
			return false;
		}
		return true;
	});
	...
	return articlesNew.slice(skip, top);
}
...
function validateArticle(article){
	if(typeof article.id != "string" || 
		typeof article.title != "string"  || article.title.lenght == 0 || article.title.lenght > 99 ||
		typeof article.summary != "string" || article.summary.lenght > 199 ||
		typeof article.createdAt != "object" ||
		typeof article.author != "string" || article.author.lenght == 0 ||
		typeof article.content != "string" || article.content.lenght == 0) return false;
	return true;
}
...
function editArticle(id, article){
	...
	if(tmp){
		for(var val in article){
			if( val == "title"){
				if(typeof article[val] == "string" && article[val].lenght != 0){
					if(article[val].lenght > 99) return false;
				...
				if(typeof article[val] == "string" && article[val].lenght != 0){
					if(article[val].lenght > 199) return false;
...
console.log(getArticles(0,3,{author: "Nova"}));
console.log(getArticles(-1,3,{author: "Nova"}));
console.log(addArticle({
		id: "11",
		title: "Министр внутренних дел рассказал",
		summary: "В интервью корреспонденту БЕЛТА министр внутренних дел Игорь Шуневич коснулся вопросов незаконной миграции.",
		img:"http://www.gorod.gomel.by/Libraries/%D0%9D%D0%BE%D0%B2%D0%BE%D1%81%D1%82%D0%B8_2017/Ohrana_logo.sflb.ashx",

...
function getArticles(skip, top, filter){
	...
	var articles = JSON.parse(JSON.stringify(db.articles.find()), (key, value) =>
...
function editArticle(article){
	var tmp = db.articles.findOne({id: article.id});
...
function validateUser(user){
	var user = db.users.findOne({login : user.login, password: user.password});
	if(user){
		db.user.save(user);
		return true;
	}
...
app.get("/articles", (req, res) => res.json(getArticles(req.query.skip, req.query.top, req.query.filter)));
